...
def main_traveller(n_ciudades, input_file_path, puntos_de_corte):
    ...
    while convergencia <= 200:
        ...
        [viajes_realizados_df, distancias_list] = seleccion_padres_y_cruce(
            viajes_realizados_df[:], distancias_list, ciudades_df[:])
        [viajes_realizados_df, distancias_list] = mutacion_sustitucion(
            viajes_realizados_df[:], ciudades_df[:], n_ciudades, distancias_list)
        best_trip = distancias_list[mejor_viajante(distancias_list)]
...
def seleccion_padres_y_cruce(viajes_df, distancias_list, ciudades_df):
    ...
    nuevos_padres = []
    nuevas_distancias = []
    nuevas_distancias.append(mejor_dist)
    ...
    for padre in nuevos_padres:
        ...
        nuevas_distancias.append(calcula_distancia(
            nuevo_viajante.tolist(), ciudades_df[:]))
        nuevos_viajantes.columns = range(0, len(nuevos_viajantes.columns))
    return [nuevos_viajantes, nuevas_distancias]


def mutacion_sustitucion(nuevos_viajantes, ciudades_df, n_ciudades, nuevas_distancias):
    ...
    for viajante in range(1, len(nuevos_viajantes.columns)):
        if(random.random() < mutation_prob):
            ...
            nuevas_distancias[viajante] = calcula_distancia(
                nuevos_viajantes.iloc[:, viajante].tolist(), ciudades_df[:])
    nuevos_viajantes.columns = range(0, len(nuevos_viajantes.columns))
    return[nuevos_viajantes, nuevas_distancias]
